...
def transits(ra, dec, jd=False, lat=Deg10(32.2217), lon=Deg10(110.9258)):
    if not jd:
        jd = julian_date(now=True)
    jd = int(jd)+0.5
    ST0 = calcGMST(jd)  # Siderial Time  0 hour UT

    alt, az = calc_AltandAz(ra, dec, jd=jd)
    H0 = Angle(math.acos((math.sin(-0.5667*math.pi/180)-math.sin(lat)
                          * math.sin(dec))/(math.cos(lat)*math.cos(dec))))
...
class locale:
    ...
    def hor2eq(self, alt, az):  # Horizontal coordinates to equatorial
        ...
        ha = Angle(math.atan2(math.sin(az), (math.cos(
            az)*math.sin(self.lat) + math.tan(alt)*math.cos(self.lat))), 'radians')
        Dec = Angle(math.asin(math.sin(self.lat)*math.sin(alt) -
                              math.cos(self.lat)*math.cos(alt)*math.cos(az)), 'radians')
    ...
    def eq2hor(self, ra, dec):  # Equatorial to horizontal coordinates
        ...
        alt = Deg10(Angle(math.asin(math.sin(self.lat)*math.sin(dec) +
                                    math.cos(self.lat)*math.cos(dec)*math.cos(ha))))
        az = Deg10(Angle(math.pi+math.atan2(math.sin(ha), math.cos(ha) *
                                            math.sin(self.lat) - math.tan(dec)*math.cos(self.lat))))  # Measured from north
    ...
    def hor2hadec(self, alt, az):
        Lat=self.lat
        ha = Angle(math.atan2(math.sin(alt), math.cos(alt) *
                              math.cos(self.lat) + math.tan(alt) * math.cos(self.lat)))
        Dec = Angle(math.asin(math.sin(Lat) * math.sin(alt) -
                              math.cos(Lat) * math.cos(alt) * math.cos(az)))
        return ha, Dec

    def hadec2hor(self, ha, dec ):

        alt = math.sin(self.lat)*math.sin(dec) + math.cos(self.lat)*math.cos(dec)*math.cos(ha)
        alt = Angle(math.asin(alt))
        try:
            az =( math.sin(dec) - math.sin(alt)*math.sin(self.lat))/(math.cos(alt)*math.cos(self.lat))
            az = Angle(math.acos(az))
    ...
    def suneq(self):  # Eq coords of sun
        ...
        RAangle = math.atan2((math.sin(Lambda)*math.cos(Epsilon) -
                              math.tan(Beta)*math.sin(Epsilon)), math.cos(Lambda))

        Decangle = math.asin(math.sin(Beta)*math.cos(Epsilon) +
...
def precess(jd_i=2451545.0, jd_f=False, calendar='J', ra=False, dec=False, obj=False):
    ...
    zeta = Deg10(((2306.2181 + 1.39656*T - 0.000139*T**2)*t +
                  (0.30188 - 0.000344*T)*t**2 + 0.017998*t**3)/3600)
    z = Deg10(((2306.2181 + 1.39656*T - 0.000139*T**2)*t +
               (1.09468 + 0.000066*T)*t**2 + 0.018203*t**3)/3600)
    theta = Deg10(((2004.3109 - 0.85330*T - 0.000217*T**2)*t -
                   (0.42665 + 0.000217*T)*t**2 - 0.041833*t**3)/3600)
